...
function paging($tabel="", $cnt = 1){
    global $connect, $_GET;

    $arr = array();

    // 전체 게시물 갯수
    $sql = "select * from {$tabel} order by idx desc";
    ...
    $sql = "select * from {$tabel} limit {$start}, {$list_num}";
    $arr['result'] = mysqli_query($connect, $sql);
